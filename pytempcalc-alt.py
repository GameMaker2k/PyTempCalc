...
def wind_chill(temp, wind_speed, temp_unit="Fahrenheit", wind_unit="MPH", new_wind_chill=True):
    temp = float(temp)
    ...
    if new_wind_chill:
        chill = 35.74 + 0.6215 * temp - 35.75 * math.pow(wind_speed, 0.16) + 0.4275 * temp * math.pow(wind_speed, 0.16)
    else:
        chill = 0.0817 * (3.71 * math.sqrt(wind_speed) + 5.81 - 0.25 * wind_speed) * (temp - 91.4) + 91.4

    return round_to_int(convert_temp_units(chill, "Fahrenheit", temp_unit))

def generate_windchill_xml(temp_unit="Fahrenheit", wind_unit="MPH", new_wind_chill=True, output_file="-"):
    ...
    for wind_speed in range(5, 65, 5):
        for temp in range(40, -46, -5):
            wind_chill_value = wind_chill(temp, wind_speed, temp_unit, wind_unit, new_wind_chill)
            result += f' <wcc windspeedmph="{wind_speed}" temperature{temp_unit.lower()}="{temp}" windchill{temp_unit.lower()}="{wind_chill_value}" />\n'
...
def generate_heat_index_xml(temp_unit="Fahrenheit", output_file="-"):
    ...
    for humidity in range(40, 105, 5):
        for temp in range(80, 112, 2):
            ...
            result += f' <hic temperature{temp_unit.lower()}="{temp}" humidity="{humidity}" heatindex{temp_unit.lower()}="{heat_index_value}" />\n'
...
def heat_index(temp, humidity, temp_unit="Fahrenheit"):
    ...
    return round_to_int(convert_temp_units(heat_index_value, "Fahrenheit", temp_unit))
